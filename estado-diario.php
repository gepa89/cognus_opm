...
if(isset($_POST["desArt"]) && $_POST["desArt"] != ''){
    $pd = str_pad(trim($_POST['desArt']), 10, '0', STR_PAD_LEFT);
    $dsAr = " and ct_empaque = '{$pd}' ";
    $fl = 1;
}else{
    $dsAr = "";
}
if($fl == 1){
    $sq = "SELECT distinct ct_empaque, lg_c_pos, lg_c_mat, lg_c_mat_desc, lg_c_cant, ct_user, ct_ts  FROM log_cierre inner join log_material on ct_empaque = lg_c_emp where 1=1 {$dsAr} {$dtd1} {$usrs}";
    //echo $sq;
    $rs = $db->query($sq);
    while($row = $rs->fetch_assoc()){
        $data_usr[$row['ct_user']][$row['ct_empaque']][$row['lg_c_pos']]['total']+=(int)$row['lg_c_cant'];
        $data_usr[$row['ct_user']][$row['ct_empaque']][$row['lg_c_pos']]['codigo']=$row['lg_c_mat'];
        $data_usr[$row['ct_user']][$row['ct_empaque']][$row['lg_c_pos']]['desc']=$row['lg_c_mat_desc'];
        $data_usr[$row['ct_user']][$row['ct_empaque']][$row['lg_c_pos']]['fech']=$row['ct_ts'];
    }
}
                            ...
                            <form method="post" id="eForm">
                                ...
                                <div class="col-lg-2 col-sm-4" >
                                    <div class="input-group">
                                        <label class="label">Documento:</label><div style="clear:both;"></div><br/>
                                        <input class="form-control" id="desArt" name="desArt" value="<?php echo $pd;?>"/>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-5 col-sm-5">
                                    ...
                                    <div class="input-group input-daterange">
                                        <input autocomplete="off" type="text" name="dFecCre" class="form-control" data-date-format="dd-mm-yyyy" placeholder="Seleccionar" value="<?php echo @$_POST["dFecCre"]?>"/>
                                        <div class="input-group-addon"> hasta </div>
                                        <input autocomplete="off" type="text" name="hFecCre" class="form-control" data-date-format="dd-mm-yyyy" placeholder="Seleccionar" value="<?php echo @$_POST["hFecCre"]?>"/>
                                    </div>
                                </div>
