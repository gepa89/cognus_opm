...
<body class="full_width">
          ...
          <form id="form_filtros" method="get">
            <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title">Administrar Aseguradoras</h3>
              </div>
        ...
        <div class="row">
          <div class="col-sm-12 col-md-12">
            ...
            echo '<table class="table">'
              . '<thead>'
              . '<tr>'
              . '<th colspan="2">Agregar Aseguradoras</th>'
              . '</tr>'
              . '</thead>'
              . '</table>';
                ...
                <div class="row">
                   <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <input type="text" id="updAse" name="updAse" class="form-control"
              ...
              <form method="post" id="eForm">
               <div class="row">
                   <div class="col-sm-4">
                    <div class="form-group">
                      <label for="buscar">Aseguradora</label>
                      <input type="text" id="addAse" name="addAse" class="form-control"
                        placeholder="" value="" />
                    </div>
                  </div>
                     <div class="col-sm-7">
  ...
  <script type="text/javascript">
          ...
          
          {
            mRender: function (data, type, row) {
              let texto = `<a title="Modificar Material">
                                            <span style="font-size:14px" onclick="updDat('${row.codseguro}','${row.nombre}','${row.fecre}')" class="glyphicon glyphicon-pencil"></span></a>
                                         <a title="Eliminar Bultos">
                                            <span style="font-size:14px" onclick="eliminarBultos('${row.codseguro}')" class="glyphicon glyphicon-trash"></span></a>`;
              return texto;
            }
          }
        ],
    ...
    $(document).ready(function () {
      $('#para43').addClass('active');
      $('#c7').click();
      let url = 'requests/obtenerAseguradora.php';
      crearTabla(url);
      $("#form_filtros").submit(function (e) {
        e.preventDefault();
        let buscar = $("#buscar").val();
        let url = `requests/obtenerAseguradora.php?buscar=${buscar}`;
        if (table != null) {
          table.destroy();
          table = null;
        }
    ...
    function saveAdd() {
      ...
      if (Ase != '') {
        $.ajax({
          type: 'POST',
          url: 'requests/saveAseguradora.php',
          data: {
            action: 'add',
            Ase: Ase,
            Des: Des,
            ...
            table: 'seguros',
            fields: 'codseguro,nombre'
          }, success: function (data) {
            var dt = JSON.parse(data);
            alert(dt.msg);
            if (dt.err == 0) {
              window.location = 'crea_aseguradora.php';
            }
