...
if($_SESSION['user'] == 'PERMARTI'){
    ...
    $grp = "and a.AFNAM = '' and a.EKGRP = (SELECT GRUPO_COMPRAS FROM SAPABAP1.ZGCOMPRAS where JEFE = '{$_SESSION['user']}' limit 1)";
}

//var_dump($grupos);
 $oci_qry = "SELECT distinct
                a.BANFN, a.ERNAM, a.BADAT, a.AFNAM, a.BEDNR,c.SAKTO,c.KOSTL,c.AUFNR, a.BEDNR
        FROM
	SAPABAP1.eban a
...
while ($row = odbc_fetch_object($result)){
    ...
    $dt = "select distinct
        ...
        where a.BANFN = '{$row->BANFN}' order by a.EBELN asc";
//                        echo $dt."<br/>";
   ...
   //            var_dump($vx);
       ...
       //                                        echo $qryprv."<br/>";
                       ...
                       if($mn == 'PYG'){
                           ...
                           $data_of[$vx->EVRTN][$vx->EVRTP]['MAYORU'] = number_format(($row6->MAYOR), 0, ',','.');
                            $data_of[$vx->EVRTN][$vx->EVRTP]['MAYOR'] = number_format(($row6->MAYOR)*(int)$data_of[$vx->EVRTN][$vx->EVRTP]['CANT'], 0, ',','.');
                            $data_of[$vx->EVRTN][$vx->EVRTP]['MENORU'] = number_format(($row6->MENOR), 0, ',','.');
                            $data_of[$vx->EVRTN][$vx->EVRTP]['MENOR'] = number_format(($row6->MENOR)*(int)$data_of[$vx->EVRTN][$vx->EVRTP]['CANT'], 0, ',','.');
