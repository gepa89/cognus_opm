...
<body class="full_width">
            ...
            <div class="main_content">
                <div class="row">
                    ...
                    echo '<table class="table">'
                        . '<thead>'
                        . '<tr>'
                        . '<th colspan="3">Gestion de Inventarios</th>'
                        . '</tr>'
                        . '</thead>'
                        . '</table>'; ?>
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-2">
                                    <div class="input-group">
                                        ...
                                        <select class="form-control" id="selCto" name="selCto[]" multiple="multiple">
                                            ...
                                            <?php
                                            $sql = "select distinct clasdoc from pedinvecab where clasdoc <> null or clasdoc <> ''";
                                            $rs = $db->query($sql);
                                            while ($ax = $rs->fetch_assoc()) {
                                                echo '<option value="' . $ax['clasdoc'] . '">' . $ax['clasdoc'] . '</option>';
                                            }
                                            ?>
                                    ...
                                    <div class="col-lg-12 col-sm-12">
                                        <div class="input-group">
                                            <input type="hidden" id="recepcion" name="recepcion" class="form-control"
                                                value="" />
                                            <input type="text" disabled id="almacen" name="almacen" class="form-control"
                                                value="" />
    ...
    <script type="text/javascript">
        ...
        function ckFields() {
            ...
            $.ajax({
                type: 'POST',
                url: 'requests/getReportInve.php',
                async: false,
                data: {
                    desPed: desPed,
                    desPrv: desPrv,
        ...
        function format(d) {
            ...
            $.each(d, function (k, v) {
                ...
                cntnt += '<td>' + v.posdoc + '</td>';
                cntnt += '<td>' + v.artrefer + '</td>';
                cntnt += '<td>' + v.artdesc + '</td>';
                cntnt += '<td>' + v.ubirefer + '</td>';
                cntnt += '<td>' + v.canubi1 + '</td>';
                cntnt += '<td>' + v.canfisi1 + '</td>';
                cntnt += '<td>' + v.diferencia1 + '</td>';
                cntnt += '<td>' + v.usuario1 + '</td>';
                cntnt += '<td>' + v.canubi2 + '</td>';
                cntnt += '<td>' + v.canfisi2 + '</td>';
                cntnt += '<td>' + v.diferencia2 + '</td>';   
                cntnt += '<td>' + v.usuario2 + '</td>';
                cntnt += '<td>' + v.canubi3 + '</td>';
                cntnt += '<td>' + v.canfisi3 + '</td>';
                cntnt += '<td>' + v.diferencia3 + '</td>';
                cntnt += '<td>' + v.usuario3 + '</td>';
