...
<body class="full_width">
          ...
          <form id="form_filtros" method="get">
            <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title">Consultas Ordenes de Expedicion Altura y Piso</h3>
              </div>
                ...
                <form method="post" id="eForm1">
                  <div class="col-lg-12 col-sm-12">
                    <div class="input-group">
                      ...
                      <input type="text" disabled id="almacen" name="almacen" class="form-control" value="" />
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      ...
      $('#tblReg tbody').on('click', '.details-control', function() {
        ...
        if (row.child.isShown()) {
          ...
          $.LoadingOverlay("show");
          $.ajax({
            url: `/wmsd/api/v1/obtener_datos_expedicion_cajas_detalles.php`,
            data: {
              pedido: rowx.pedido
            },
            dataType: 'json',
    ...
    function obtenerUrl() {
      let url = '/wmsd/api/v1/obtener_datos_expedicion_cajas.php?';
    ...
    function generar_tabla_detalles(pedido, datos, row) {
      ...
      let columna_detalles = [{
          ...
          data: 'cantidad_preparada',
          title: 'Cant. Preparada',
          className: 'dt-body-center'
        },
        {
          data: 'zonas',
          ...
          columns: [{
            ...
            },
            {
              ...
              mRender: function(data, type, row) {
                ...
                if (row.clase_documento == 'REPO') {
                  if (row.terminal_reposicion != "") {
                    group += `<span>${row.terminal_reposicion}</span>`;
                  }
                  if (row.zona_reposicion) {
                    group += `<button class=\"btn btn-primary btn-fnt-size\"  
                    onclick=\"asReceptionRe('${row.pedido}','REPO','${row.zona_reposicion}','${row.almacen}')\">Asignar Repo</button>`;
                  }
                } else {
                  for (let index = 0; index < row.zonas.length; index++) {
                    const zona = row.zonas[index];
                    let btnAsignar = "";
                    if (zona.estado == 'libre') {
                      btnAsignar = `<button class=\"btn btn-primary btn-fnt-size\"  onclick=\"asReception('${row.pedido}','${zona.zona}','${row.almacen}','REPO')\">Asignar - ${zona.zona}</button>`;
                    } else {
                      btnAsignar = `<span>${zona.terminal}</span>`;
                    }
                    group += btnAsignar;
                  }
                }
            ...
            },
            {
              ...
              mRender: function(data, type, row) {
                let group = `<div>`;
                if (row.enviar_sap == "ignorar") {
                  return "";
                } else if (row.enviar_sap == "enviado") {
                  group += "<span>Enviado</span>";
                } else if (row.enviar_sap == "enviar") {
                  group += `<button class=\"btn btn-warning btn-fnt-size\"  
                  onclick=\"sendExpedition('${row.pedido}')\">Enviar</button>`;
                }
                group += "</div>";
                return group;
              }
            },
            {
              ...
              mRender: function(data, type, row) {
                let group = `<div class="btn-group" role="group">`;
                if (row.habilitar_anulacion) {
                  group += `<button class=\"btn btn-danger btn-fnt-size\"  
                  onclick=\"nullDoc('${row.pedido}','Ex')\">Anular Pedido</button>`;
                }
                group += "</div>";
                return group;
              }
