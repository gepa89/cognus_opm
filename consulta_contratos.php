...
<body class="full_width">
          ...
          <form id="form_filtros" method="get">
            <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title">Consultar Contratos Marcos</h3>
              </div>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      ...
      $('#tblReg tbody').on('click', '.details-control', function() {
        ...
        if (row.child.isShown()) {
          ...
          $.LoadingOverlay("show");
          $.ajax({
            url: `api/v1/obtener_contratos_detalle.php`,
            data: {
              pedido: rowx.idcontra
            },
            dataType: 'json',
    ...
    function obtenerURL() {
      let url = '/api/v1/obtener_contratos.php?';
        ...
        table = $('#tblReg').DataTable({
          ...
          columns: [{
            ...
            },
            {
              ...
              className: 'dt-body-center',
              mRender: function(data, type, row) {
                let progrButton = `<li><a style="margin:0;padding:0" onclick="programarHorario('${row.idcontra}')" href="javascript:void(0);">Sol.Aprobacion</a></li>`;
                let anularButton = `<li><a style="margin:0;padding:0"  onclick="anularPedido('${row.idcontra}')"  href="javascript:void(0);">Anular</a></li>`;
                let cerrarButton = `<li><a style="margin:0;padding:0"  onclick="cerrarPedido('${row.idcontra}')"  href="javascript:void(0);">Cerrar</a></li>`;
    ...
    function anularPedido(idcontra) {
      let url = "/api/v1/anular_oferta.php?idcontra=" + idcontra;
      let contenido = $("#anular-descarga").html();
      contenido = contenido.replace('__pedido__', idcontra);
      contenido = contenido.replace('__url__', url);

      generarModal(`Anular Descarga ${idcontra}`, contenido);
    }

    function cerrarPedido(idcontra) {
