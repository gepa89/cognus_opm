...
$centro = $_ENV['GENERAL']['CENTRO'];
$aml = $_ENV['GENERAL']['ALMACEN'];
...
while ($rw = odbc_fetch_object($rst)) {
    ...
    if (!in_array($cod, $ldd)) {
        ...
        //$data[$cod]['cab']['obse1'] = obtenerMensajePedido($rw->VGBEL);
        $data[$cod]['cab']['clidir'] = ''; //todo: completar este campo desde sap
        ...
        $data[$cod]['det'][$cc]['artdesc'] = $db->real_escape_string(iconv('ISO-8859-1', 'UTF-8', $rw->ARKTX));
...
foreach ($data as $cod => $dat) {
    ...
    foreach ($dat['det'] as $detalle) {
        if (
            ...
            $res = $db->query($sql_cabecera_cajas);
            if (!$res) {
    ...
    if ($db->query($sqx)) {
        ...
        if (!$is_ok) {
            guardar_error_log("ingreso_expedicion", $sqd . $values, $db->error);
            $db->rollback();
            exit;
        } else {
            $db->commit();
...
function obtenerMensajePedido($id_pedido)
{
    $con_sap = SapRFCConnection::obtenerInstancia();
    try {
        //    echo $data['vgbel']." - ";
...
{
    // verifica flag para dividir pedidos
