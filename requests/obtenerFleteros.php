...
if ($buscar) {
    ...
    $filtro_buscar = " AND codflete like '%$buscar%' OR codflete like '%$buscar%'";
}
try {

    $sql = "SELECT codflete,fecre, nombre FROM fleteros WHERE TRUE {$filtro_buscar} order by fecre DESC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM fleteros WHERE TRUE {$filtro_buscar} order by fecre DESC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
