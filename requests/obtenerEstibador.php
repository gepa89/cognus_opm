...
if ($buscar) {
    ...
    $filtro_buscar = " AND codestib like '%$buscar%' OR codestib like '%$buscar%'";
}
try {

    $sql = "SELECT codestib,fecre, nombre FROM estibador WHERE TRUE {$filtro_buscar} order by fecre DESC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM estibador WHERE TRUE {$filtro_buscar} order by fecre DESC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
