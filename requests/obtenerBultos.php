...
if ($buscar) {
    ...
    $filtro_buscar = " AND tipbul like '%$buscar%' OR tipbul like '%$buscar%'";
}
try {

    $sql = "SELECT tipbul,fecre, nombre FROM bultos WHERE TRUE {$filtro_buscar} order by fecre DESC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM bultos WHERE TRUE {$filtro_buscar} order by fecre DESC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
