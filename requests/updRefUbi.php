...
$msgx = '';
    foreach($row as $k => $v){ 
        ...
        $onbd = getDataOnBd($v[4], $db);//obtiene el registro en bd de que corresponde a la fila     
//        var_dump($onbd);
        ...
        if($arrDiff){
            ...
            if($origen){ //si la ubicación actual está vacia
                if($ubi == ''){ //si se borra la ubicación de destino intenta eliminar si es que está vacia
                    $del = "delete from artubiref where refid = {$v[4]}";
...
//                        echo $del;
                    ...
                    if($destino['err'] == 1){
                        ...
                        $sqin = "update artubiref set ubirefer = '{$ubi}', ubitipo = '{$tipo}' where refid = {$v[4]}";
//                        echo $sqin;
                        $db->query($sqin);
                    }
                }
