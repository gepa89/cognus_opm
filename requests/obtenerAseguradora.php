...
if ($buscar) {
    ...
    $filtro_buscar = " AND codseguro like '%$buscar%' OR codseguro like '%$buscar%'";
}
try {

    $sql = "SELECT codseguro,fecre, nombre FROM seguros WHERE TRUE {$filtro_buscar} order by fecre DESC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM seguros WHERE TRUE {$filtro_buscar} order by fecre DESC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
