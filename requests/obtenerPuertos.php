...
if ($buscar) {
    ...
    $filtro_buscar = " AND codpuerto like '%$buscar%' OR codpuerto like '%$buscar%'";
}
try {

    $sql = "SELECT codpuerto,fecre, nompuerto FROM puertos WHERE TRUE {$filtro_buscar} order by fecre DESC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM puertos WHERE TRUE {$filtro_buscar} order by fecre DESC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
