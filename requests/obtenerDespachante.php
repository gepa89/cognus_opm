...
if ($buscar) {
    ...
    $filtro_buscar = " AND codespa like '%$buscar%' OR codespa like '%$buscar%'";
}
try {

    $sql = "SELECT codespa,fecre, nombre FROM despachantes WHERE TRUE {$filtro_buscar} order by fecre DESC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM despachantes WHERE TRUE {$filtro_buscar} order by fecre DESC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
