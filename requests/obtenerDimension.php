...
if ($buscar) {
    ...
    $filtro_buscar = " AND dimension like '%$buscar%'";
}
try {

    $sql = "SELECT dimension, dimdesc, codalma FROM dimen WHERE TRUE {$filtro_buscar} order by dimension ASC LIMIT $limite offset $offset";
    $query = $db->query($sql);
    if (!$query) {
    ...
    $sql = "SELECT COUNT(*) as cantidad FROM dimen WHERE TRUE {$filtro_buscar} order by dimension ASC";
    $query = $db->query($sql);
    if (!$query) {
        guardar_error_log(__FILE__, $db->error);
        $db->close();
