...
<body class="full_width">
          ...
          <form id="form_filtros" method="get">
            <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title">Programar Descargas</h3>
              </div>
              <div class="panel-body">
                <div class="row">
                  ...
                  <div class="col-md-2">
                   ...
                   <div class="col-md-2">
                    <div class="form-group">
                      <label for="buscar">Fecha Descarga Desde:</label>
                      <input class="form-control" id="fecha_descard" " name="fecha_descard" type="date">
                    </div>
                  </div>
                  <div class="col-md-2">
                    <div class="form-group">
                      <label for="buscar">Fecha Descarga Hasta:</label>
                      <input class="form-control" id="fecha_descarh" " name="fecha_descarh" type="date">
                    </div>
                  </div> 
  ...
  <script id="form-horario-descarga" type="text/template">
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="form-group">
          ...
          <input type="date" required class="form-control" value="__fecha_descarga__" id="fecha_descarga" name="fecha_descarga" />
        </div>
        <div class="form-group">
          <label for="buscar"></label>
          <Button class="btn btn-primary btn-submit" type="submit">
            <img id="loading-button" hidden src="/wmsd/assets/img/loading.svg" width="20px" alt="">
            Guardar
          </Button>
        </div>
      </div>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      ...
      $('#tblReg tbody').on('click', '.details-control', function() {
        ...
        if (row.child.isShown()) {
          ...
          $.LoadingOverlay("show");
          $.ajax({
            url: `/wmsd/api/v1/obtener_datos_promdesca_detalles.php`,
            data: {
              pedido: rowx.pedido
            },
            dataType: 'json',
    ...
    function obtenerURL() {
      let url = '/wmsd/api/v1/obtener_datos_promdesca.php?';
      url += `pedido=${$("#pedido").val()}&`;
      url += `proveedor=${$("#proveedor").val()}&`;
      url += `fecha_desde=${$("#fecha_desde").val()}&`;
      url += `fecha_hasta=${$("#fecha_hasta").val()}&`;
      url += `fecha_descard=${$("#fecha_descard").val()}&`;
      url += `fecha_descarh=${$("#fecha_descarh").val()}&`;
        ...
        table = $('#tblReg').DataTable({
          ...
          columns: [{
            ...
            },
            {
              ...
              className: 'dt-body-center',
              mRender: function(data, type, row) {
                let progrButton = `<li><a style="margin:0;padding:0" onclick="programarHorario('${row.pedido}')" href="javascript:void(0);">Programar</a></li>`;
    ...
    function programarHorario(pedido) {
      let url = "/wmsd/api/v1/guardar_programacion_descarga.php?pedido=" + pedido;
      let contenido = $("#form-horario-descarga").html();
    ...
    function anularPedido(pedido) {
      let url = "/wmsd/api/v1/anular_descarga.php?pedido=" + pedido;
    ...
    function cerrarPedido(pedido) {
      let url = "/wmsd/api/v1/cerrar_descarga_programada.php";
