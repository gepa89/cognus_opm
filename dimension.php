          ...
          <form id="form_filtros" method="get">
            <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title">Crear Dimensiones</h3>
              </div>
        ...
        <div class="row">
          <div class="col-sm-12 col-md-12">
            ...
            echo '<table class="table">'
              . '<thead>'
              . '<tr>'
              . '<th colspan="2">Agregar Dimension</th>'
              . '</tr>'
              . '</thead>'
              . '</table>';
                ...
                <div class="col-lg-12 col-sm-12">
                  ...
                  <div class="col-lg-4 col-sm-4">
                    <div class="input-group">
                      ...
                      <input type="text" id="updDim" name="updDim" class="form-control" placeholder="" value="" />
                    </div>
                  </div>  
                  <div class="col-lg-4 col-sm-4">
                    <div class="input-group">
                ...
                <div class="col-lg-12 col-sm-12">
                  ...
                  <div class="col-lg-8 col-sm-4">
                    <div class="input-group">
                      <label class="label" style="color:#000;">Dimension:</label>
                      <div style="clear:both;"></div><br />
                      <input type="text" id="addDim" name="addDim" class="form-control" placeholder="" value="" />
                    </div>
                  </div>  
                  <div class="col-lg-8 col-sm-4">
                    <div class="input-group">
                      <label class="label" style="color:#000;">Descripcion:</label>
                      <div style="clear:both;"></div><br />
                      <input type="text" id="addDes" name="addDes" class="form-control" placeholder="" value="" />
                    </div>
                  </div>
  ...
  <script type="text/javascript">
    ...
    function crearTabla(url) {
      table = $('#tbl_list').DataTable({
        ...
        columns: [
          { data: 'dimension' },
          { data: 'dimdesc', orderable: false },
          { data: 'codalma' },
          {
            mRender: function (data, type, row) {
              let texto = `<a title="Modificar Material">
                                            <span style="font-size:14px" onclick="updDat('${row.dimension}','${row.dimdesc}','${row.codalma}')" class="glyphicon glyphicon-pencil"></span></a>
                                         <a title="Modificar Material">
                                            <span style="font-size:14px" onclick="eliminarEan('${row.dimension}','${row.dimdesc}','${row.codalma}')" class="glyphicon glyphicon-trash"></span></a>`;
              return texto;
            }
          }
        ],
    ...
    $(document).ready(function () {
      $('#capa2').addClass('active');
      $('#p1').click();
      let url = 'requests/obtenerDimension.php';
      crearTabla(url);
      $("#form_filtros").submit(function (e) {
        e.preventDefault();
        let buscar = $("#buscar").val();
        let url = `requests/obtenerDimension.php?buscar=${buscar}`;
        if (table != null) {
          table.destroy();
          table = null;
        }
    ...
    function updDat(addDim, addDes, addAlm) {
    ...
    function eliminarEan(dim, cod_alma) {
      $.ajax({
        type: 'POST',
        url: 'requests/eliminarDimension.php',
        data: {
          dim: dim,
          cod_alma: cod_alma,
    ...
    function saveAdd() {
      ...
      if (Dim != '') {
        $.ajax({
          type: 'POST',
          url: 'requests/saveDimension.php',
          data: {
            action: 'add',
            Dim: Dim,
            Des: Des,
            Alm: Alm,
            table: 'dimen',
            fields: 'dimension, dimdesc, codalma'
          }, success: function (data) {
            var dt = JSON.parse(data);
            alert(dt.msg);
            if (dt.err == 0) {
              window.location = 'dimension.php';
            }
