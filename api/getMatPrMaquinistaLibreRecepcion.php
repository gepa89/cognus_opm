...
if ($puede_mezclar_articulo) {
    ...
    $parametros = [$fecha_ingreso_pedido, $material, $cantidad_dias_mezcla];
    $sql = "SELECT * FROM(SELECT
            ...
            stockubi.fecingre,
            capaubi.capacidad,
            (select sum(canti) from stockubi where ubirefer=stockubi.ubirefer) as
            cantidad_disponible
            from
            stockubi
            inner join ubimapa ON ubimapa.ubirefer = stockubi.ubirefer
            inner join capaubi on capaubi.artrefer=stockubi.artrefer and
            capaubi.dimension=ubimapa.dimension
            where
            stockubi.artrefer = ? and ubimapa.ubitipo='PS'
            order by stockubi.fecingre asc limit 1) as datos
            where diferencia <= ? and cantidad_ocupada > 0 and capacidad > cantidad_ocupada";
    ...
    if ($ubicacion_encontrada) {
        $resultado = generarRespuesta($cod_alma, $ubicacion_encontrada, $detalle_pedido, $db);
        $db->close();
...
$sql = "SELECT
                ...
                (select
                        COUNT(*)
                    from
                        ( -- verificamos si hay artuculos distintos ubicados en esa ubicacion
                        select
...
guardar_info_log("ubicacion en piso", $sql);
$sqlbuilder = new MySqlQuery($db);
...
function generarRespuesta($cod_alma, $lugar, $detalle_pedido, $db = null)
{
    ...
    $resultado->capacidad_ubicacion = obtenerCapacidadUbicacion($lugar->ubirefer, $detalle_pedido->artrefer);
