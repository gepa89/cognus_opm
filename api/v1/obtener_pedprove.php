...
if ((int) $order_by_id > 0) {
    ...
    if ($parametro_ordenar == "fecre") {
        $ordenar_por['horcre'] = $parametro_ordenar_direccion;
    }
}
...
if ($buscar) {
    ...
    if (is_numeric($buscar)) {
        $filtros_busqueda .= " OR docompra LIKE '%$buscar%' ";
    }

    if (!is_numeric($buscar)) {
        $filtros_busqueda .= " OR clasdoc LIKE '%$buscar%' OR ($sql_terminal) LIKE '%$buscar%' OR situped LIKE '%$buscar%'";
    }
...
$sql_condicion = " FROM
                    pedidosincab
                    left join proveedores on pedidosincab.codprove = proveedores.codprove
...
$sql_datos = "
    SELECT 
    pedidosincab.docompra,
    pedidosincab.clasdoc,
    pedidosincab.codsocie,
    pedidosincab.codmone,
    proveedores.nombre,
    pedidosincab.fecre,
    pedidosincab.orgcompra,
    pedidosincab.grupcompra,
    pedidosincab.situped,
    SUM(pedidosindet.pretotal) as totalp,
    pedidosincab.userliberacion
    

        $sql_condicion
        
        GROUP BY pedidosincab.docompra
        ORDER BY pedidosincab.docompra asc
        LIMIT ? OFFSET ?
";
//print $sql_datos;
