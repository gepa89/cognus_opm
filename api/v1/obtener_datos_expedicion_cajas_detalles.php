...
$sql = "SELECT DISTINCT
        ...
        artipresen.preseref AS presentacion,
        artipresen.canpresen AS cantidad_presentacion
        FROM pedexdet detalles_pedido
        INNER JOIN arti articulo on detalles_pedido.artrefer=articulo.artrefer 
        left join artipresen on detalles_pedido.artrefer = artipresen.artrefer
        WHERE detalles_pedido.pedexentre = '$pedido' and artipresen.preseref='CJ'";
...
foreach ($datos as &$dato) {
    ...
    $dato['cantidad_pedido'] = formatear_numero(calcularCantidadPresentacion($dato['cantidad_pedido'], $dato['cantidad_presentacion'])) . " CJ";
    $dato['cantidad_pendiente'] = formatear_numero(calcularCantidadPresentacion($dato['cantidad_pendiente'], $dato['cantidad_presentacion'])) . " CJ";
    $dato['cantidad_preparada'] = formatear_numero(calcularCantidadPresentacion($dato['cantidad_preparada'], $dato['cantidad_presentacion'])) . " CJ";
