...
$ordernar_por  = "pedexcab.pedexfec";
...
if ($buscar) {
    ...
    if (is_numeric($buscar)) {
        $filtros_busqueda .= " OR pedexentre LIKE '%$buscar%' OR pedexcab.codenv = '%$buscar%' OR ped_multiref.multiref LIKE '%$buscar%'";
    }

    if (!is_numeric($buscar)) {
        $filtros_busqueda .= " OR clientes.clinom LIKE '%$buscar%' OR pedexcab.siturefe LIKE '%$buscar%' OR pedexcab.pedclase LIKE '%$buscar%' OR ped_multiref.terminal LIKE '%$buscar%'";
    }
...
$sql_condicion = " FROM
                    ...
                    left join (
                        ...
                        )	as clientes on clientes.clirefer=pedexcab.clirefer
                    LEFT JOIN ped_multiref on pedexcab.pedexentre = ped_multiref.pedido and ped_multiref.pedcajas = '0'
                    left join mref_exp on
                        ped_multiref.multiref = mref_exp.mref
                        and mref_exp.pedido = pedexcab.pedexentre
                    WHERE pedexcab.almrefer='$almacen' $filtro $filtros_busqueda";

$sql = " SELECT
            pedexcab.pedsent as enviado_sap,
            pedexcab.send,
            pedexcab.pedexentre as pedido,
            pedexcab.almrefer as almacen,
            pedexcab.pedclase as clase_documento,
            clientes.clinom as nombre_cliente,
            pedexcab.pedexfec as fecha_creacion,
            pedexcab.pedexhor as hora_creacion,
            pedexcab.codenv as cod_envio,
            mref_exp.ubirefer as ubicacion,
            pedexcab.siturefe as situacion,
            ped_multiref.terminal as terminal,
...
function obtener_zonas($materiales)
{
    ...
    foreach ($materiales as $pedido => $material) {
        ...
        inner join ubimapa b on
            a.ubirefer = b.ubirefer and b.ubitipo IN ('PI','RE','PS')
            and a.cod_alma = b.cod_alma
        where
            a.artrefer in ('{$codigos_cadena}')
        GROUP BY
            b.zoncodpre,a.artrefer
        HAVING sum(a.canti) > 0";
        $sqlBuilder = new MySqlQuery();
        $resultados = $sqlBuilder->rawQuery($sqzon, [])->getAll();

        foreach ($resultados as $resultado) {
            if (verificar_cajas($pedido, $resultado['artrefer'], $resultado['zoncodpre'])) {
...
function verificar_cajas($pedido, $material, $zona)
{
    $db = MysqlDB::obtenerInstancia();
    $sqlBuilder = new MySqlQuery($db);

    $sql = "SELECT * from config WHERE estaparam=0";
    $configuracion = $sqlBuilder->rawQuery($sql, [])->getOne();
    if ($configuracion) {
        return true;
    }
    ...
    $sql = "SELECT * from pedexdet  
        ...
        where pedexentre = '$pedido' and pedexdet.artrefer='$material' and artipresen.preseref = 'CJ'";

    $dato = $sqlBuilder->rawQuery($sql, [])->getOne();
    if ($dato == null) {
        return true;
    }

    // verificamos stock hay stock en altura mayor a la cantidad restante, si hay pasa  la siguiente validacion sino muestra boton unidades
    $sql = "SELECT
        a.artrefer,
        sum(a.canti) as cantidad 
        from
            stockubi a
        inner join ubimapa b on
            a.ubirefer = b.ubirefer and b.ubitipo='RE'
            and a.cod_alma = b.cod_alma
        where
            a.artrefer = '$material'
            and b.zoncodpre = '$zona'
        GROUP BY
            b.zoncodpre,a.artrefer
        HAVING sum(a.canti) > 0";
    $sqlBuilder = new MySqlQuery($db);
    $resultado = $sqlBuilder->rawQuery($sql, [])->getOne();
    /* si no hay stock en altura mostrar boton */
    $cantidad_restante = $dato->canpedi - $dato->canprepa;
    $cantidad_presentacion = $dato->canpresen;
    if ($resultado == null || ($resultado->cantidad < $cantidad_presentacion)) {
        return true;
    }
    $db->close();
    return $cantidad_restante % $cantidad_presentacion != 0;
}
function obtener_zonas_ocupadas($pedidos)
{
    $zonas_ocupadas = [];
    $codigos_cadena = implode("','", $pedidos);
    $sql = "select zona,tercod,pedido from assig_ped where pedido IN ('$codigos_cadena') and pedcajas is null";
    $sqlBuilder = new MySqlQuery();
...
function obtener_terminales_asignadas_por_zona($pedidos)
{
    $datos = [];
    $codigos_cadena = implode("','", $pedidos);
    $sql = "SELECT * FROM assig_ped WHERE pedcajas IS NULL AND pedido IN ('$codigos_cadena')";
    $sqlBuilder = new MySqlQuery();
    $resultados = $sqlBuilder->rawQuery($sql, [])->getAll();
