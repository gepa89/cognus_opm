...
function actualizarCabeceraPedidos($db, $codigo_pedido)
{
    ...
    if ($resultado->fetch_object()->cantidad == 0) {
        ...
        $db->query($sql);
        $sqlbuider = new MySqlQuery($db);
        $condicion = ['pedido' => $codigo_pedido];
        $parametros = [
            'codst' => 1,
        ];
        $stmt = $sqlbuider->table('ped_multiref')->buildUpdate($parametros)->where($condicion)->executeUpdate();
        if ($stmt->errno) {
            $db->rollback();
            $db->close();
            $response = array('exito' => false, 'mensaje' => 'Error al actualizar movimiento de articulos');
            return retorna_resultado($response, 500);
        }
...
function actualizarStockUbicacion(
    ...
    $sql = "UPDATE
            ...
            WHERE
                ...
                AND stockubi.cod_alma = '$cod_almacen'
                AND stockubi.ubirefer = '$ubicacion_ref'
                {$filtro_lote}";

    $res = $db->query($sql);
    $registro_actualizado = $db->affected_rows > 0;
    if (!$res) {
        $db->rollback();
        retorna_resultado(array("mensaje" => "error al actualizar stok ubi"), 500);
    }
    if ($existe && !$registro_actualizado) {
    ...
    $sql = "DELETE FROM stockubi WHERE
    stockubi.artrefer = '$codigo_articulo'
    AND stockubi.cod_alma = '$cod_almacen'
    AND stockubi.ubirefer = '$muelle'
    and stockubi.etnum='$multireferencia'
    and stockubi.canti = 0
    {$filtro_lote}";
    $res = $db->query($sql);
    if (!$res) {
        guardar_error_log(__FILE__, $db->error);
        $db->rollback();
        retorna_resultado(array("mensaje" => "error al eliminar stok ubi"), 500);
    }
}
