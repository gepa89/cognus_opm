...
<body class="full_width">
          ...
          <form id="form_filtros" method="get">
            <div class="panel panel-info">
              <div class="panel-heading">
                <h3 class="panel-title">Consultas Ordenes de Expedicion</h3>
              </div>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      ...
      $('#tblReg tbody').on('click', '.details-control', function() {
        ...
        if (row.child.isShown()) {
          ...
          $.LoadingOverlay("show");
          $.ajax({
            url: `/wmsdz/api/v1/obtener_datos_expedicion_detalles.php`,
            data: {
              pedido: rowx.pedido
            },
            dataType: 'json',
    ...
    function obtenerUrl() {
      let url = '/wmsdz/api/v1/obtener_datos_expedicion.php?';
                ...
                if (row.clase_documento == 'REPO') {
                  ...
                  for (let index = 0; index < row.zonas.length; index++) {
                    ...
                    if (zona.estado == 'libre') {
                      btnAsignar = `<button class=\"btn btn-primary btn-fnt-size\"  onclick=\"asReception('${row.pedido}','${zona.zona}','${row.almacen}','EXPE')\">Asignar - ${zona.zona}</button>`;
                    } else {
